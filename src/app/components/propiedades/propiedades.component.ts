...
export class PropiedadesComponent {
  ...
  Propiedades(value: any, property: keyof CanvasComponent['style']) {
    ...
    if (
      property === 'inputType' &&
      this.selectedComponent.type === 'checklist'
    ) {
      this.updateChildrenInputType(formattedValue);
    }
  ...
  setBorderProperty(prop: 'width' | 'style' | 'color', value: string | number) {
    ...
    const newBorder = `${width || '0'} ${style || 'solid'} ${
      color || '#000000'
    }`;
  ...
  addOption() {
    if (!this.selectedComponent || this.selectedComponent.type !== 'select')
      return;
  ...
  removeOption(index: number) {
    if (
      !this.selectedComponent?.children ||
      !this.selectedComponent.children[index]
    )
      return;
  ...
  updateTableStructure() {
    if (!this.selectedComponent || this.selectedComponent.type !== 'table')
      return;
  ...
  addTableRow() {
    ...
    const columnCount =
      this.selectedComponent.children[0]?.children?.length || 1;
  ...
  //metodos par checklist
  updateChildrenInputType(newType: string) {
    if (!this.selectedComponent?.children) return;

    // Si cambias a radio, asegurar que solo uno esté marcado
    if (newType === 'radio') {
      ...
      this.selectedComponent.children.forEach((child) => {
        ...
        if (child.checked && !hasCheckedItem) {
          hasCheckedItem = true;
        } else if (child.checked && hasCheckedItem) {
          child.checked = false;
        }
        ...
        this.SokectSevice.updateComponentProperties(
          ...
          this.selectedPageId!,
          child.id,
          { type: newType, checked: child.checked }
        );
      });
    } else {
      // Para checkbox, mantener estados checked
      this.selectedComponent.children.forEach((child) => {
        child.type = newType;
  ...
  // 5. Modifica addItemCheck para usar el inputType del style:

  addItemCheck() {
    if (!this.selectedComponent || this.selectedComponent.type !== 'checklist')
      return;

    // Usar el inputType del style del checklist
    const inputType = this.selectedComponent.style.inputType || 'checkbox';
